...
## 6b. SDK-Based LLM Deployment

Rather than managing local quantized inference on edge hardware, the system will use an SDK-based LLM deployment. Using a robust cloud or API-based SDK removes the memory bottlenecks of running local models (like `llama_ros`) alongside the rest of the robotics stack, freeing up critical computational resources for the MPC loop and the MuJoCo simulation. 

This approach ensures you have access to the latest, most capable reasoning models for dispatch and situational awareness without fighting the 6GB VRAM constraint of your development machine, aligning with standard industry practice for robot reasoning and planning layers.
...
- Which task template/pipeline applies (pour, wipe, stack, open, delicate grasping via Deli Grasp, etc. — your established kitchen task families plus the contact-rich ones from Phase 4).
...
- The LLM inference runs via SDK-based deployment with a capable cloud API, ensuring high reliability for parsing and parameterizing tasks.
